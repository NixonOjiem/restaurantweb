        ...
        <div v-if="imagePreviews.length > 0" class="space-y-2">
          ...
          <div class="grid grid-cols-2 gap-3 max-h-[400px] overflow-y-auto custom-scrollbar">
            <div v-for="(preview, index) in imagePreviews" :key="index"
              ...
              <!-- Primary Badge -->
              <div v-if="index === 0"
                class="absolute bottom-2 left-2 px-2 py-0.5 bg-indigo-600 text-white text-xs font-bold rounded">
                Main
              </div>
            </div>
          </div>
      ...
      <div class="flex-1 flex flex-col">
        <div class="p-6 space-y-5 flex-1 overflow-y-auto custom-scrollbar">
          ...
          <div>
            ...
            <p class="text-sm text-gray-500">{{ isEditMode ? 'Update the product details below' :
          ...
          <div>
            ...
            <input v-model="formData.title" type="text" required placeholder="e.g., Margherita Pizza"
          ...
          <div>
            ...
            <textarea v-model="formData.description" required rows="4" placeholder="Describe your product..."
          ...
          <div class="grid grid-cols-2 gap-4">
            <div>
              ...
              <input v-model.number="formData.price" type="number" required min="0" step="0.01" placeholder="0.00"
                class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-all">
            </div>
            <div>
              <label class="block text-sm font-semibold text-gray-700 mb-2">
              ...
              <input v-model.number="formData.discountPrice" type="number" min="0" step="0.01" placeholder="0.00"
                class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-all">
            </div>
          </div>
          ...
          <div class="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input v-model="formData.sku" type="text" placeholder="e.g., PROD-001"
                class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-all">
            </div>
          </div>
          ...
          <div>
            ...
            <div class="space-y-2">
              <div v-for="(ingredient, index) in formData.ingredients" :key="index" class="flex gap-2">
                <input v-model="formData.ingredients[index]" type="text" placeholder="e.g., Tomatoes"
          ...
          <div>
            ...
            <div class="grid grid-cols-2 gap-3">
              <label v-for="option in dietaryOptions" :key="option" class="flex items-center gap-2 cursor-pointer">
                <input type="checkbox" :value="option" v-model="formData.dietaryInfo"
          ...
          <div>
            ...
            <div class="space-y-2">
              <div v-for="(badge, index) in formData.badge" :key="index" class="flex gap-2">
                <input v-model="formData.badge[index]" type="text" placeholder="e.g., Best Seller"
                  class="flex-1 px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-all">
        ...
        <div class="p-4 bg-white border-t border-gray-200 flex gap-3">
          ...
          <button type="submit" :disabled="isSubmitting"
            ...
            <span>{{ isSubmitting ? (isEditMode ? 'Updating...' : 'Creating...') : (isEditMode ? 'Update Product' :
              'Create Product') }}</span>
          </button>
        </div>
...
const handleSubmit = async () => {
  ...
  try {
    ...
    // 2. Prepare FormData
    // We cannot send JSON directly when uploading files. We must use FormData.
    const formDataToSend = new FormData();

    // Append the file (Key 'images' matches the backend route configuration)
    ...
    // IMPORTANT: Stringify arrays so they can travel via FormData
    ...
    // The browser sets 'multipart/form-data' automatically when it sees FormData.

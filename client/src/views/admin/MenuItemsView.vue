    ...
    <section class="border border-gray-300 rounded-lg p-4 bg-white shadow-sm space-y-4">
      ...
      <div v-for="product in products" :key="product._id" class="group relative flex w-full flex-col overflow-hidden rounded-xl bg-white shadow-sm border border-gray-100 transition-all duration-300 hover:shadow-lg hover:-translate-y-0.5 hover:border-indigo-200">
        ...
        <div class="absolute top-2 right-2 z-1 flex items-center gap-1.5">
          <span v-for="dietaryInfo in product.dietaryInfo" :key="dietaryInfo" class="px-2 py-1 text-xs font-semibold text-white bg-green-600/80 rounded-full">{{ dietaryInfo }}</span>
        </div>
      ...
      <div>
        <span class="text-xs rounded-full px-2 py-1 border" :class="categoryColors[product.category as keyof typeof categoryColors]">{{ product.category }}</span>
